"""
Security Scanner - Deep security checks
"""
...
def run_security_checks(connections):
    """
    Run deep security checks on the system
    """
    checks = []
    
    # Check 1: SSH on standard port
    checks.append(check_ssh_port(connections))
    
    # Check 2: Common vulnerable ports
    checks.append(check_dangerous_ports(connections))
    
    # Check 3: Too many listening ports
    checks.append(check_listening_count(connections))
    
    # Check 4: External connections on sensitive ports
    checks.append(check_sensitive_external(connections))
    
    # Check 5: Unusual outbound connections
...
def check_listening_count(connections):
    """Check if too many ports are listening"""
...
def check_sensitive_external(connections):
    """Check for external connections to sensitive ports"""
    sensitive_ports = [22, 3306, 5432, 6379, 27017, 9200]  # SSH, MySQL, PostgreSQL, Redis, MongoDB, Elasticsearch
